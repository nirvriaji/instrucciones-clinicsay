    ...
    case 'add_template': {
      ...
      if (flowName && updated.toolOrchestration.flows[flowName]) {
        updated.toolOrchestration.flows[flowName] = {
          ...updated.toolOrchestration.flows[flowName],
            responseTemplateKey: 'generic_processed',
          };
        updated.responseTemplates = {
          ...(updated.responseTemplates ?? {}),
          generic_processed: {
            text: 'Perfecto, hemos procesado tu solicitud. ¿Necesitas algo más?',
            mode: 'model',
          },
        };
        changes.push(`Added responseTemplateKey and registry entry to flow '${flowName}'`);
      } else {
