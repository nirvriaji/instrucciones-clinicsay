...
export type WireFlow = {
  ...
  responseTemplateMode?: 'literal' | 'model' | null;
...
export function fromWireFormat(wire: WireStructuredLogic): StructuredLogic {
  ...
  for (const entry of wire.toolOrchestration.flows) {
    ...
    if (entry.responseTemplate != null) flow.responseTemplate = entry.responseTemplate;
    if (entry.responseTemplateMode != null) flow.responseTemplateMode = entry.responseTemplateMode;
...
export function validateWireStructuredLogic(wire: WireStructuredLogic): WireValidationResult {
  ...
  if (!wire.toolOrchestration || typeof wire.toolOrchestration !== 'object') {
    ...
    wire.toolOrchestration.flows.forEach((flow, index) => {
      ...
      } else if (flow.steps.length === 0 && !flow.responseTemplate && flow.allowsSilence !== true) {
        errors.push(
          `toolOrchestration.flows[${index}].steps is empty but has no responseTemplate or allowsSilence. ` +
            `Conversation-only flows must provide a responseTemplate.`
        );
      }
    });
  }
