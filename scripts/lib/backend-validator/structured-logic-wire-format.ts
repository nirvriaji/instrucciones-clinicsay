...
import type { StructuredLogic, IntentDefinition, ToolFlow, Protocol, ResponseTemplate, GlobalSchedulingPolicy } from './structured-logic';
...
export type WireFlow = {
  ...
  steps: Array<{
    ...
    when: Array<{ key: string; equals?: string | null; in?: string[] | null; notIn?: string[] | null; exists?: boolean | null }> | null;
    note?: string | null;
  }>;
      ...
      flows: logic.toolOrchestration?.flows
        ? Object.entries(logic.toolOrchestration.flows).map(([flowName, flow]) => ({
            ...
            steps: flow.steps.map((step) => ({
              step: step.step, tools: step.tools, parallel: step.parallel, required: step.required ?? null,
              customState: step.customState ? step.customState.map((field) => ({ key: field.key, description: field.description, enum: field.enum ?? null })) : null,
              when: step.when ? step.when.map((condition) => ({ key: condition.key, equals: condition.equals ?? null, in: condition.in ?? null, notIn: condition.notIn ?? null, exists: condition.exists ?? null })) : null,
              note: step.note ?? null,
            })),
  ...
  for (const entry of wire.toolOrchestration.flows) {
    const flow: ToolFlow = {
      ...
      steps: entry.steps.map((s) => {
        ...
        if (s.customState != null) step.customState = s.customState.map((field) => ({ key: field.key, description: field.description, ...(field.enum != null ? { enum: field.enum } : {}) }));
        if (s.when != null) step.when = s.when.map((condition) => ({ key: condition.key, ...(condition.equals != null ? { equals: condition.equals } : {}), ...(condition.in != null ? { in: condition.in } : {}), ...(condition.notIn != null ? { notIn: condition.notIn } : {}), ...(condition.exists != null ? { exists: condition.exists } : {}) }));
