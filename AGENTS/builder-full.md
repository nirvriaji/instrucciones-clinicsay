...
13. **Investigación de leads con comportamiento inesperado.** Si un lead nuevo presenta un comportamiento anómalo (ej. no agenda cuando debería, crea tarea en lugar de cita, responde fuera de contexto, cancela sin motivo), solicita al asesor que te proporcione el `structured-logic.full.json` actualmente en producción en el backend. Compara ese JSON con el archivo local del repo para detectar discrepancias (diferencias de flows, tools faltantes, reglas divergentes, intents desactualizados). El asesor puede obtenerlo desde el dashboard del backend o solicitándolo al equipo técnico.
